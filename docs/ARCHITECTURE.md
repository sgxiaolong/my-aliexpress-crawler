...
本文档阐述 `my-aliexpress-crawler` 项目的设计初衷、架构分层原则、Git 子模块管理与会话状态持久化方案。

---

## 1. 为什么采用 “主业务工程 + Git 子模块” 架构？
...
└── libs/
    └── aliexpress-product-scraper/    <-- 底层开源抓取核心库 (纯上游代码，0 侵入修改)
```

1. **业务主工程** 负责 HTTP 接口层设计、重试队列、会话生命周期维护与凭证持久化。
2. **`libs/aliexpress-product-scraper`** 以原生 Git Submodule 形式挂载，无需在其源码内部夹杂任何特定的业务配置逻辑。
...
### 本项目的“零侵入固化”机制
1. **浏览器挂载持久目录**：无论是 HTTP 服务 (`server.js`) 还是人工交互脚本 (`interactive_scraper.js`)，启动 Puppeteer 时均挂载主工程下的 `user_data_profile/`；
2. **主动注入 + 自动保存**：读取外部 `cookie.txt` 或 API 传来的新 Cookie 注入网页；
3. **Chromium 原生数据库存储**：注入后只要网页正常发起请求，Chrome 引擎底层会自动将凭证写入 `user_data_profile/Default/Network/Cookies` SQLite 数据库中。
4. 后续任何任务运行只需指明 `userDataDir: "./user_data_profile"` 即可持久享有认证状态。
